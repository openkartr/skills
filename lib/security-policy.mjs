...
const safeDisplayNamePattern = /^[\p{L}\p{N}][\p{L}\p{N}\p{M} _./,&+:#@()!?'-]{0,99}$/u;
...
function hasSafeManifestName(frontmatterSource, manifestSource) {
  const frontmatterName = frontmatterSource?.match(/^name:\s*(.+?)\s*$/m)?.[1];
  const headingName = manifestSource.match(/^#\s+([^\r\n]+?)\s*$/m)?.[1];
  return [frontmatterName, headingName].some(
    (value) => typeof value === "string" && safeDisplayNamePattern.test(unquoteYamlScalar(value)),
  );
}
...
export function scanNormalizedBundle(files, { trustTier = "community" } = {}) {
  ...
  const manifestSource = skillFile.contents.toString("utf8");
  const frontmatter = manifestSource.match(/^---\r?\n([\s\S]*?)\r?\n---/);
  assert(
    frontmatter && hasSafeManifestName(frontmatter[1], manifestSource),
    "SKILL.md needs a safe name in YAML frontmatter or its first-level heading.",
  );
