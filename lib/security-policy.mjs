...
export function scanNormalizedBundle(files, { trustTier = "community" } = {}) {
  ...
  const safeFrontmatterName = frontmatter && /^name:\s*["']?[A-Za-z0-9][A-Za-z0-9 _.-]{0,99}["']?\s*$/m.test(frontmatter[1]);
  const safeHeadingName = /^#\s+[A-Za-z0-9][A-Za-z0-9 _.-]{0,99}\s*$/m.test(manifestSource);
  assert(
    frontmatter && (safeFrontmatterName || safeHeadingName),
